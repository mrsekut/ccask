			...
			const showList = async () => {
				try {
					...
					const stats = {
						...
						generating: allQuestions.filter((q) => q.status === "generating")
							.length,
						completed: allQuestions.filter((q) => q.status === "completed")
							.length,
					...
					if (status !== "all")
						filters.push(`${formatStatus(status as QuestionStatus)}`);
