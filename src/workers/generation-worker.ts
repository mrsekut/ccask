...
import { updateQuestionStatus, getQuestionByTerm } from "../core/question-manager.js";
...
const logMessage = async (message: string): Promise<void> => {
  ...
  try {
    await writeFile("ask.log", logEntry, { flag: "a" });
  } catch (error) {
...
const updateQuestionFromWorker = async (term: string, status: "generating" | "completed" | "failed", updates: any = {}): Promise<void> => {
  try {
...
const generateExplanation = async (args: ProcessedWorkerArgs): Promise<void> => {
  ...
  await updateQuestionFromWorker(question, "generating", { startedAt: new Date().toISOString() });
  ...
  child.on("close", async (code) => {
    if (code === 0 && output.trim()) {
      try {
        ...
        await logMessage(`Successfully generated explanation for: ${question} -> ${filepath}`);
      } catch (writeError) {
      ...
      await logMessage(`Failed to generate explanation for ${question}. ${errorMsg}`);
    }
  });
