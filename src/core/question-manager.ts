...
export const loadQuestionsDB = async (): Promise<QuestionsDB> => {
  await ensureDataDir();
...
export const saveQuestionsDB = async (db: QuestionsDB): Promise<void> => {
  await ensureDataDir();
...
export const updateQuestionStatus = async (
  ...
  updates: Partial<Pick<Question, "startedAt" | "completedAt" | "filepath" | "error">> = {}
): Promise<void> => {
  const db = await loadQuestionsDB();
...
export const getQuestionByTerm = async (term: string): Promise<Question | undefined> => {
  const db = await loadQuestionsDB();
...
export const getQuestionsByStatus = async (status: QuestionStatus): Promise<Question[]> => {
  const db = await loadQuestionsDB();
...
export const getAllQuestions = async (): Promise<Question[]> => {
  const db = await loadQuestionsDB();
  return db.questions.sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
};