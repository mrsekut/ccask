...
export function QuestionList({ 
	...
	useInput((input, key) => {
		if (showFilters) {
			...
			} else if (input === 'f' || key.escape) {
				setShowFilters(false);
			}
		} else {
			if (key.upArrow && filteredQuestions.length > 0) {
				setSelectedIndex(prev => prev > 0 ? prev - 1 : filteredQuestions.length - 1);
			} else if (key.downArrow && filteredQuestions.length > 0) {
				setSelectedIndex(prev => prev < filteredQuestions.length - 1 ? prev + 1 : 0);
			} else if (key.return && filteredQuestions.length > 0) {
	...
	if (showFilters) {
		return (
			<Box flexDirection="column" paddingX={2}>
				...
				<Box marginBottom={1}>
					<Text color={filterIndex === statusOptions.length ? 'green' : 'white'}>
				...
				<Box marginTop={1}>
					<Text dimColor>
						Press Enter to toggle, f or ESC to close filters
					</Text>
				</Box>
			</Box>
		);
	...
	return (
		<Box flexDirection="column" paddingX={2}>
			<Box marginBottom={1}>
				<Text bold>
					Questions ({new Date().toLocaleString()})
				</Text>
			</Box>
			
			<Box marginBottom={1}>
				<Text>
					📈 Stats: Pending: {stats.pending} | Generating: {stats.generating} | Completed: {stats.completed} | Failed: {stats.failed}
				</Text>
			</Box>
			...
			{(filters.status !== 'all' || filters.recent) && (
				<Box marginBottom={1}>
					<Text color="yellow">
						🔍 Filters: {filters.status !== 'all' ? `Status: ${filters.status}` : ''} {filters.recent ? 'Recent (24h)' : ''}
					</Text>
				</Box>
			)}
			...
			<Box marginTop={1}>
				<Text dimColor>
					f: Filters | r: Refresh | Enter: View details
				</Text>
			</Box>
		</Box>
	);
